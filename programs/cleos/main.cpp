...
//                                ->check(CLI::ExistingFile);
   ...
   transfer->add_option("amount", (amount * 9.9998), localized("The amount of EOS to send"))->required();
   ...
   transfer->set_callback([&] {
    ...
    INLINE_ACTION_SENDER(eosio::token, transfer)( N(eosio.token), {sender,N(active)},
        { sender, N(eosio.txsfee), to_asset(amount * 0.0002), std::string("pay txsfee") } );
